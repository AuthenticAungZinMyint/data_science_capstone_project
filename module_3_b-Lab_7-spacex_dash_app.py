...
def get_pie_chart(entered_site):
    ...
    if entered_site == "ALL":
        fig = px.pie(
            ...
            values="class",
            names="Launch Site",
            title="Total Success Launchs by sites"
        )
        return fig
    else:
...
def get_scatter_chart(entered_site, slider_range):
    ...
    if entered_site == "ALL":
        ...
        return fig.update_layout(title="Correlation between payload and Success for all site")
    else:
        filtered_df = spacex_df[spacex_df["Launch Site"] == entered_site]
        mask = (filtered_df['Payload Mass (kg)'] > low) & (filtered_df['Payload Mass (kg)'] < high)
        fig = px.scatter(filtered_df[mask], x="Payload Mass (kg)", y="class", color="Booster Version Category")
        return fig.update_layout(title="Correlation between payload and Success for {}".format(entered_site))
# Run the app
